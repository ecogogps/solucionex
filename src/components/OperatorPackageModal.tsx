...
Respaldo y Seguridad en cada entrega.`;
  ...
  // NUEVA FUNCIÓN PARA LA EMPRESA CON EL NOMBRE DEL OPERADOR
  const getCompanyWhatsAppUrl = (pkg: PaqueteData) => {
    if (!pkg.empresas?.telefono) return '';
    const phone = pkg.empresas.telefono.replace(/^0/, '').replace(/\D/g, '');
    const nombreOperador = pkg.operadores?.nombres || '';
    const message = `Soy el operador ${nombreOperador}`;
    return `whatsapp://send?phone=593${phone}&text=${encodeURIComponent(message)}`;
  };
            ...
            <>
              ...
              <div className="bg-white/5 p-4 rounded-xl border border-white/10 space-y-3">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    {/* Botones de contacto a la empresa */}
                    {selectedPackage.empresas?.telefono && (
                      <div className="flex items-center gap-2 shrink-0">
                        <Button 
                          variant="outline" 
                          size="icon" 
                          className="h-9 w-9 border-white/10 text-accent hover:bg-white/10" 
                          asChild
                        >
                          <a 
                        ...
                        <Button 
                          variant="outline" 
                          size="icon" 
                          className="h-9 w-9 border-[#25D366]/30 bg-[#25D366]/10 text-[#25D366] hover:bg-[#25D366]/20" 
                          asChild
                        >
                          <a 
                            href={getCompanyWhatsAppUrl(selectedPackage)} 
                            target="_blank" 
                            rel="noopener noreferrer"
                            title={`WhatsApp ${selectedPackage.empresas.nombre}`}
                          >
